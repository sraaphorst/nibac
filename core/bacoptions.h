...
//
// As the number of options for the branch-and-cut algorithm
// was getting overwhelming, these were moved to a separate
// class where they could be given sensible defaults, as opposed
// to passing in everything to the constructor to BAC and MargotBAC.
//
// $Author$
// $Date$
...
public:
  ...
  inline void setMinimumNumberOfCuts(int pCP_MIN_NUMBER_OF_CUTS) {
    if (pCP_MIN_NUMBER_OF_CUTS < 0)
      throw IllegalParameterException("BACOptions::MinimumNumberOfCuts", pCP_MIN_NUMBER_OF_CUTS, "MinimumNumberOfCuts must be nonnegative");
  ...
  inline void setMinimumViolationL(double pCP_MIN_VIOLATIONL) {
    if (pCP_MIN_VIOLATIONL < 0)
      throw IllegalParameterException("BACOptions::MinimumViolationL", pCP_MIN_VIOLATIONL, "MinimumViolationL must be nonnegative");
  ...
  inline void setMinimumViolationU(double pCP_MIN_VIOLATIONU) {
    if (pCP_MIN_VIOLATIONU < 0)
      throw IllegalParameterException("BACOptions::MinimumViolationU", pCP_MIN_VIOLATIONU, "MinimumViolationU must be nonnegative");
  ...
  inline void setViolationToleranceL(double pCP_VIOLATION_TOLERANCEL) {
    if (pCP_VIOLATION_TOLERANCEL < 0)
      throw IllegalParameterException("BACOptions::ViolationToleranceL", pCP_VIOLATION_TOLERANCEL, "ViolationToleranceL must be nonnegative");
  ...
  inline void setViolationToleranceU(double pCP_VIOLATION_TOLERANCEU) {
    if (pCP_VIOLATION_TOLERANCEU < 0)
      throw IllegalParameterException("BACOptions::ViolationToleranceU", pCP_VIOLATION_TOLERANCEU, "ViolationToleranceU must be nonnegative");
  ...
  inline void setActivityTolerance(double pCP_ACTIVITY_TOLERANCE) {
    if (pCP_ACTIVITY_TOLERANCE < 0)
      throw IllegalParameterException("BACOptions::ActiviyTolerance", pCP_ACTIVITY_TOLERANCE, "ActivityTolerance must be nonnegative");
