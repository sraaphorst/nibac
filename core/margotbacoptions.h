...
//
// A subclass of BACOptions which includes some options
// specific for Margot's algorithms.
//
// $Author$
// $Date$
...
class MargotBACOptions : public BACOptions
{
protected:
...
public:
  ...
  inline void setOrbitThreshold(int porbitThreshold) {
    if (porbitThreshold < 0)
      throw IllegalParameterException("MargotBACOptions::OrbitThreshold", porbitThreshold, "OrbitThreshold must be nonnegative");
...
protected:
  ...
  // Declare MargotBAC a friend so we can call this protected method.
