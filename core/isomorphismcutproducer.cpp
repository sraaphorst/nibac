					  ...
					  int &numberOfCuts, double &maximumViolation)
{
  ...
  for (iter = cuts; iter;) {
    Constraint *constraint = Constraint::createConstraint(formulation, iter->getNumberIndices(), iter->getIndices(), LESSTHAN, iter->getNumberIndices()-1);
...
// *** ISOMORPHISMCUTPRODUCERCREATOR methods ***
...
std::map< std::string, std::pair< std::string, std::string > > IsomorphismCutProducerCreator::getOptionsMap(void)
{
  ...
  optionsMap[std::string("M")] =
    std::pair< std::string, std::string >(std::string("Minimum fractional value: only consider variables for isomorphism "
