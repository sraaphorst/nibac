    ...
    int CommandLineProcessing::populateBACOptions(int &argc, char **&argv) {
        ...
        for (int i = 0; i < argc;) {
            ...
            if (strcmp(argv[i], "-n") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-n", "none",
                                                    "-n requires a minimum number of cuts to be specified");
                int paramvalue = atoi(argv[i + 1]);
                if (paramvalue < 0)
            ...
            if (strcmp(argv[i], "-m") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-m", "none",
                                                    "-m requires a lower bound on violation to be specified");
            ...
            if (strcmp(argv[i], "-M") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-M", "none",
                                                    "-M requires an upper bound on violation to be specified");
                double paramvalue = atof(argv[i + 1]);
                if (paramvalue < 0.0)
            ...
            if (strcmp(argv[i], "-v") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-v", "none",
                                                    "-v requires a lower bound on required violation to be specified");
                double paramvalue = atof(argv[i + 1]);
                if (paramvalue < 0.0)
                    throw IllegalParameterException("-v", paramvalue,
                                                    "lower bound on required violation must be nonnegative");
            ...
            if (strcmp(argv[i], "-V") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-V", "none",
                                                    "-V requires an upper bound on required violation to be specified");
                double paramvalue = atof(argv[i + 1]);
                if (paramvalue < 0.0)
                    throw IllegalParameterException("-V", paramvalue,
                                                    "upper bound on required violation must be nonnegative");
            ...
            if (strcmp(argv[i], "-b") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-b", "none",
                                                    "-b requires a lower bound on objective function to be specified");
            ...
            if (strcmp(argv[i], "-B") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-B", "none",
                                                    "-B requires an upper bound on objective function to be specified");
            ...
            if (cutProducerCreators.size() > 0 && (argv[i][0] == '+' || argv[i][0] == '-') && argv[i][1] == 'c') {
                ...
                if (!stream)
                    if (argv[i][0] == '-')
                        throw IllegalParameterException("-c", "none",
                                                        "-c must have the form -c#, where # specifies the cut type number");
                    else
                        throw IllegalParameterException("+c", "none",
                                                        "+c must have the form +c#option=value:option=value:..., where # "
            ...
            if (solutionManagerCreators.size() > 1 && argv[i][0] == '-' && argv[i][1] == 'S') {
                ...
                stream >> paramvalue;
                if (!stream)
                    throw IllegalParameterException("-S", "none",
                                                    "-S must have the form -S#option=value:option=value:..., where # "
            ...
            if (variableOrderCreators.size() > 1 && argv[i][0] == '-' && argv[i][1] == 'o') {
                ...
                stream >> paramvalue;
                if (!stream)
                    throw IllegalParameterException("-o", "none",
                                                    "-o must have the form -o#option=value:option=value:..., where # "
            ...
            if (branchingSchemeCreators.size() > 1 && argv[i][0] == '-' && argv[i][1] == 'R') {
                ...
                stream >> paramvalue;
                if (!stream)
                    throw IllegalParameterException("-R", "none",
                                                    "-R must have the form -s#option=value:option=value:..., where # "
            ...
            if (strcmp(argv[i], "-f") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-f", "none",
                                                    "-f requires a string list of 0-fixings to be specified");
            ...
            if (strcmp(argv[i], "-F") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-F", "none",
                                                    "-F requires a string list of 1-fixings to be specified");
                options.setManualFixings(true);
    ...
    int CommandLineProcessing::populateMargotBACOptions(int &argc, char **&argv) {
        ...
        for (int i = 0; i < argc;) {
            ...
            if (strcmp(argv[i], "-f") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-C", "none",
                                                    "-C requires a string list of ranges of tree depths to test for canonicity");
            ...
            if (strcmp(argv[i], "-O") == 0) {
                if (i == argc - 1)
                    throw IllegalParameterException("-O", "none",
                                                    "-O requires a string list of ranges of tree depths to 0-fix");
    ...
    void CommandLineProcessing::outputOptions(std::ostream &out) {
        ...
        if (cutProducerCreators.size() > 1) {
            ...
            for (std::map<int, CutProducerCreator *>::iterator iter = cutProducerCreators.begin();
                ...
                std::map<CutProducerCreator *, bool>::iterator diter = cutProducerCreatorDefaults.find(
                        cutProducerCreator);
