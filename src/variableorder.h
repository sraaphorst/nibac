...
//
// This file provides an abstract class to induce an ordering
// on a set of variables. This is required to establish initial variable
// orderings when using the Margot B&C scheme (variable orderings are
// irrelevant in the regular vanilla B&C). The implication of variable
// orderings are that they impact the structure of the Schreier-Sims scheme
// table, which can affect the runtime of the problem by, for example
// changing the effort required to down / reverse-down and changing which
// variables are selected by minimum index branching.
//
// All ordering classes should extend this one. Then, program authors
// who wish to use an ordering function should register the ordering
// function with the static member register, which will call the parent
// and populate the map of VariableOrder. Using the register function
// allows the programmer to determine which ordering functions are
// available, how they should be designated via the command line, and
// also allows various configuation (i.e. the public register method
// in the subclass could contain some user information).
//
// $Author$
// $Date$
...
// Class forward for VariableOrderCreator.
