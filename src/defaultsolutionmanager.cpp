...
namespace vorpal::nibac {
    DefaultSolutionManager::DefaultSolutionManager(Formulation::ProblemType p, Formulation::SolutionType s)
        ...
        if (p == Formulation::PROBLEMTYPE_UNDEFINED)
            throw IllegalParameterException("DefaultSolutionManager::ProblemType", "undefined",
                                            "Must specify if maximization / minimization problem.");
        if (s == Formulation::SOLUTIONTYPE_UNDEFINED)
            throw IllegalParameterException("DefaultSolutionManager::SolutionType", "undefined",
                                            "Must specify type of problem.");
    ...
    DefaultSolutionManager::getSolutions() {
        return solutions;
    }
    ...
    bool DefaultSolutionManagerCreator::processOptionsString(const char *options) {
        ...
        while (!stream.eof()) {
            ...
            // TODO: What exactly was I thinking here?!?
            switch (ch) {
                default:
