...
void BAC::solve()
{
  ...
  while ((node = nodeStack->getNextNode())) {
	...
	int numberNodesSolved = LPSolver::getInstance()->solveNode(*this, *node, node->getDepth() >= options.getDepth());
	...
	if (formulation.getProblemType() == Formulation::MAXIMIZATION) {
	  ...
	  if (baseSolutionValue > upperBound) {
	    validSubtreeFlag = false;
#ifdef DEBUG
	...
	if (formulation.getSolutionType() != Formulation::MAXIMALGENERATION && formulation.getSolutionType() != Formulation::ALLGENERATION) {
	  if (formulation.getProblemType() == Formulation::MAXIMIZATION && baseSolutionValue < bestSolutionValue) {
	    validSubtreeFlag = false;
#ifdef DEBUG
	  ...
	  if (formulation.getProblemType() == Formulation::MINIMIZATION && baseSolutionValue > bestSolutionValue) {
	    validSubtreeFlag = false;
#ifdef DEBUG
	  ...
	  if (formulation.getSolutionType() == Formulation::SEARCH && baseSolutionValue == bestSolutionValue) {
	    validSubtreeFlag = false;
#ifdef DEBUG
	...
	if (isInteger) {
	  ...
	  if (formulation.getProblemType() == Formulation::MAXIMIZATION) {
	    ...
	    if (equals(node->getSolutionValue(), lowerBound)) {
#ifdef DEBUG
...
void BAC::determineViolationBounds(int numberNonInteger, double &minimumViolation, double &violationTolerance)
{
  ...
  minimumViolation = options.getMinimumViolationL() + (options.getMinimumViolationU() - options.getMinimumViolationL()) * frac;
  violationTolerance = options.getViolationToleranceL() + (options.getViolationToleranceU() - options.getViolationToleranceL()) * frac;
}
...
bool BAC::isMaximal(Node &node)
{
  ...
  for (int i=0; i < numberVariables; ++i) {
    ...
    if ((formulation.getProblemType() == Formulation::MAXIMIZATION && lessthan(fractionalValue, node.getSolutionValue()))
	|| (formulation.getProblemType() == Formulation::MINIMIZATION && greaterthan(fractionalValue, node.getSolutionValue())))
      continue;
...
bool BAC::fixVariableTo1(Node &node, int variable, bool ignored)
{
...
bool BAC::fixVariableTo0(Node &node, int variable, bool ignored)
{
