...
namespace vorpal::nibac {
    /**
     * This is the abstract superclass of cut generators, e.g. isomorphism cuts, clique inequalities, etc.
     */
    class CutProducer {
    protected:
    ...
    /**
     * Basic class to create / destroy cut producers. This allows users to populate the possible options
     * in an instance of this class at will and then request that the class be created. This is really
     * only intended for use with CommandLineProcessing and is not necessary for alternative NIBAC use.
     *
     * How this works is that you create an instance of a Creator class for each cut producer you want
     * to make available to the user, and register it with the command line processing. The command line
     * processing is then performed, and then if this cut is selected, pass the corresponding options string
     * to it to have it configure itself. Pending that, the user can finalize any configuration needed (e.g.
     * number of variables), and then the command line processing finalize method is called to finish configuration.
     * This will give a BACOptions / MargotBACOptions object completely ready for use with NIBAC, requiring
     * little - if any - programmer intervention, and the command line params will be standardized from application
     * to application. Of course, it is not required that this be used and one could opt to configure manually.
     *
     * The ID identifies the cut in the sequence of cuts. The user then selects it by invoking -c# "configstring",
     * where # is the ID.
     *
     * If any data is missing from the class, a NIBAC MissingDataException should be thrown when create is called.
     */
