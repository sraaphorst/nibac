...
namespace vorpal::nibac {
    ...
    void CliqueCutProducer::generateCuts(BAC &bac, Node &node, double violationTolerance,
        ...
        for (int i = startIndex; i < endBound; ++i) {
            ...
            if (solutionVariableArray[baseElement] >= CC_FRACTIONAL_THRESHOLD &&
                solutionVariableArray[baseElement] <= upperBound)
        ...
        for (std::vector<std::vector<int> >::iterator iter = cliques.begin();
            ...
            Constraint *constraint = Constraint::createConstraint(formulation, positions, LT, 1);
    ...
    std::map<std::string, std::pair<std::string, std::string> > CliqueCutProducerCreator::getOptionsMap(void) {
        ...
        optionsMap[std::string("P")] =
                std::pair<std::string, std::string>(
                        std::string("Fractional preference: in constructing cliques, preference "
        ...
        optionsMap[std::string("E")] =
                std::pair<std::string, std::string>(
                        std::string("Enumeration value: if a component is this size or smaller, "
